...
class Node {
    ...
    init(data: [String]) {
        for (n, v) in data.enumerated() {
            if n == 0 {
                name = v
            } else if n == 1 {
                let a = v.dropFirst().dropLast()
                weight = Int(a) ?? 0
            } else {
                children.append(Node(name: v))
            }
        }
    }
}
extension Node: CustomStringConvertible {
    var description: String {
        var components = [name, String(weight)]
        if let parent = parent {
            components.append("(\(parent.name))")
        }
        if !children.isEmpty {
            components.append(children.map { $0.name }.joined(separator: "."))
        }
...
// traverse
func walkChildren(node: Node, level: Int = 0) -> Int {
    var prefix = ""
    for _ in 0...level {
        prefix += "__"
    }
//    print("\(prefix)\(node)")
    ...
    for child in node.children {
        childrenWeight += walkChildren(node: find(node: child.name)!, level: level + 1)
    }
//    if prefix == "__" || prefix == "____" {
//    if node.parent?.name == "dwggjb" {
    if node.parent?.name == "wknuyhc" {
//    if node.parent?.name == "egbzge" {
        print("\(prefix)\(node)=\(node.weight + childrenWeight)")
    }
    return node.weight + childrenWeight
}
let weight = walkChildren(node: root)
print("root:\(weight)")
