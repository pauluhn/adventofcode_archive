...
private class Solution {
    func calculateMinimumHP(_ dungeon: [[Int]]) -> Int {
        let (nodes, links) = buildGraph(dungeon)
        let start = nodes.first!
        let princess = nodes.last!
        
        var g = [Node: Score]()
        g[start] = Score(minHP: 1, currentHP: 1)
        
        for node in nodes {
            let parents = links
                .filter { $0.to.point == node.point }
                .map { $0.from }
            guard !parents.isEmpty else { continue }
            
            g[node] = parents
                .compactMap { g[$0] }
                .map { node.score(initial: $0) }
                .sorted { $0.minHP < $1.minHP }
                .first!
        }
        return g[princess]!.minHP
    }
    
    private func buildGraph(_ dungeon: [[Int]]) -> (nodes: [Node], links: [Link]) {
        let start = Node(point: Node.Point(x: -1, y: 0), val: 0) // start off the board
        
        var nodes = [start]
        var links = [Link]()
        for (y, row) in dungeon.enumerated() {
            for (x, val) in row.enumerated() {
                let point = Node.Point(x: x, y: y)
                let node = Node(point: point, val: val)
                nodes.append(node)
                // left
                if let left = nodes.node(at: Node.Point(x: x - 1, y: y)) {
                    links.append(Link(from: left, to: node))
                }
                // up
                if let up = nodes.node(at: Node.Point(x: x, y: y - 1)) {
                    links.append(Link(from: up, to: node))
                }
            }
        }
        return (nodes.sorted { $0.sum < $1.sum }, links) // nodes are sorted diagonally
    }
}
...
private struct Node: Hashable {
    ...
    let point: Point
    let val: Int
    var sum: Int { return point.sum }
    
    func score(initial score: Score) -> Score {
        guard val < 0 else { return score.increaseCurrent(by: val) }
        guard score.currentHP + val < 1 else { return score.increaseCurrent(by: val) }
        let increaseBy = 1 - score.currentHP - val
        return Score(minHP: score.minHP + increaseBy, currentHP: 1)
    }
}
...
private struct Score {
    ...
    func increaseCurrent(by val: Int) -> Score {
        return Score(minHP: minHP, currentHP: currentHP + val)
    }
}

private extension Array where Element == Node {
    func node(at point: Node.Point) -> Node? {
        return first { $0.point == point }
    }
}

extension Node.Point: CustomStringConvertible {
