...
// all guard overlap minute
var allGuardMinutes = [Int: [Int]]()
var start = 0
for line in lines {
    switch line.state {
    case .begin:
    ...
    case .wake:
        guard line.hour == 0 else { fatalError() }
        let aGuardMinutes = (start..<line.minute).map { $0 }
        if allGuardMinutes[guardId] != nil {
            allGuardMinutes[guardId]?.append(contentsOf: aGuardMinutes)
        } else {
            allGuardMinutes[guardId] = aGuardMinutes
        }
    }
}
let allGuardTheMinute: [(Int, Int, Int)] = allGuardMinutes.map {
    let countSet = NSCountedSet(array: $0.value)
    let theMinuteHit = countSet.map { ($0, countSet.count(for: $0)) }
        .sorted { $0.1 > $1.1 }
        .first!
    return ($0.key, theMinuteHit.0 as! Int, theMinuteHit.1)
}
let answer = allGuardTheMinute.sorted { $0.2 > $1.2 }.first!
