...
class Timestamp {
    ...
    var minute = 0
    var guardId = 0
    enum State: String { case begin, sleep, wake }
    var state = State.begin
}
extension Timestamp: CustomStringConvertible {
    var description: String {
        return "\(year)-\(month)-\(day) \(hour):\(minute) \(guardId) \(state)"
    }
}
extension Timestamp: Comparable {
    private func combine() -> Int {
        return year * 100 * 100 * 100 * 100
            + month * 100 * 100 * 100
            + day * 100 * 100
            + hour * 100
            + minute
    ...
    static func == (lhs: Timestamp, rhs: Timestamp) -> Bool {
        return lhs.combine() == rhs.combine()
    }
    static func < (lhs: Timestamp, rhs: Timestamp) -> Bool {
        return lhs.combine() < rhs.combine()
    }
...
let lines = input.components(separatedBy: .newlines)
    ...
    .filter { $0.count == 2 }
    .map { strings -> Timestamp in
        let regex = try! NSRegularExpression(pattern: "^\\[(\\d+)-(\\d+)-(\\d+)\\s(\\d+):(\\d+)")
        let range = NSRange(location: 0, length: strings[0].count)
        guard let match = regex.firstMatch(in: strings[0], options: [], range: range) else { fatalError() }
        var results = [Int]()
        for i in 1..<match.numberOfRanges {
            let result = (strings[0] as NSString).substring(with: match.range(at: i))
            results.append(Int(result)!)
        }
        guard results.count == 5 else { fatalError() }
        let timestamp = Timestamp()
        timestamp.year = results[0]
        timestamp.month = results[1]
        timestamp.day = results[2]
        timestamp.hour = results[3]
        timestamp.minute = results[4]
        
        switch strings[1].trimmingCharacters(in: .whitespaces).first {
        case "G"?:
            let guardId = strings[1].split(separator: " ").filter { $0.first == "#" }.first?.dropFirst()
            timestamp.guardId = Int(guardId!)!
        case "f"?: timestamp.state = .sleep
        case "w"?: timestamp.state = .wake
        default: fatalError()
        }
        return timestamp
    }
...
for line in lines {
    switch line.state {
    case .begin: guardId = line.guardId
    default: line.guardId = guardId
    }
}
// calculate most minutes
var guardMinutes = [Int: Int]()
var start = 0
for line in lines {
    switch line.state {
    case .begin:
        guardId = line.guardId
    case .sleep:
        guard line.hour == 0 else { fatalError() }
        start = line.minute
    case .wake:
        guard line.hour == 0 else { fatalError() }
        let sleep = line.minute - start
        if let minutes = guardMinutes[guardId] {
            guardMinutes[guardId] = minutes + sleep
        } else {
            guardMinutes[guardId] = sleep
        }
    }
}
let mostGuardMinutes = guardMinutes.sorted { $0.value > $1.value }.first!
let mostGuard = mostGuardMinutes.key
let mostMinutes = mostGuardMinutes.value
...
for line in lines {
    ...
    case .begin:
        guardId = line.guardId
    case .sleep:
        guard guardId == mostGuard else { continue }
        start = line.minute
    case .wake:
        guard guardId == mostGuard else { continue }
        minutes.append(contentsOf: (start..<line.minute).map { $0 })
    }
}
let countSet = NSCountedSet(array: minutes)
let theMinuteHit = countSet.map { ($0, countSet.count(for: $0)) }
    .sorted { $0.1 > $1.1 }
    .first!

// final answer
let theMinute = theMinuteHit.0 as! Int
let answer = mostGuard * theMinute
